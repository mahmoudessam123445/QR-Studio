...
(function() {
    ...
    function generateQR() {
        ...
        setTimeout(() => {
            try {
                ...
                // Logo overlay (manual, not via library)
                updateLogoOverlay();
    ...
    function downloadPNG() {
        ...
        if (state.logoImage) {
            ...
            logoImg.onload = () => {
                ...
                finishDownload(exportCanvas);
            };
            logoImg.onerror = () => finishDownload(exportCanvas);
            logoImg.src = state.logoImage;
        } else {
            finishDownload(exportCanvas);
        }

        function finishDownload(cvs) {
    ...
    function printQR() {
        ...
        if (state.logoImage) {
            ...
            logoImg.onload = () => {
                ...
                ctx.fillRect(logoX - 2, logoY - 2, logoSize + 4, logoSize + 4);
                ctx.drawImage(logoImg, logoX, logoY, logoSize, logoSize);
                doPrint(exportCanvas);
            };
            logoImg.onerror = () => doPrint(exportCanvas);
            logoImg.src = state.logoImage;
        } else {
            doPrint(exportCanvas);
        }

        function doPrint(cvs) {
            ...
            printWindow.document.write(`
                ...
                <body style="display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#fff;">
                    <img src="${cvs.toDataURL('image/png')}" style="max-width:80%;max-height:80%;" />
                </body></html>
            `);
    ...
    function handleLogoUpload(file) {
        ...
        reader.onload = (e) => {
            ...
            updateLogoOverlay();
            generateQR(); // Regenerate QR with logo overlay
